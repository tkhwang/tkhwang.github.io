...
- Ch.7에서 정의한 `ContentUrl`, `Tag`, `ConsumptionStatus` 같은 Value Object Spec을 TDD로 구현한다
- Red → Green → Refactor 사이클을 `ContentUrl` 또는 `Tag`부터 처음부터 끝까지 보여준다
- Part 1의 마무리 챕터

**그다음: Ch.9 작성**

- Ch.6에서 잡은 북마크/소비 추적 컨텍스트를 기준으로 Building Block 상세 설명을 시작한다
- Value Object를 시작으로 Entity, Aggregate로 점진적으로 확장한다
...
본편 요구사항은 [`docs/0watchable.PRD.md`](./0watchable.PRD.md)를 기준으로 한다. 시리즈 진행 현황과 다음 작업은 이 문서에서 함께 관리한다.
