...
- Ch.1~5의 `Customer` 예제는 도입용이다
- Ch.6부터는 본편 도메인(콘텐츠 북마크 & 소비 추적)으로 전환한다
...
| Ch.6 | Value Object를 TDD로 구현하기 | `next` | - | 새 도메인 전환 + Value Object SDD/TDD |
...
- Ch.6부터 본편 도메인(콘텐츠 북마크 & 소비 추적)으로 전환한다
- Ch.6 도입부에서 "도입 예제 → 본편 예제" 전환을 명시한다
...
- 새 도메인(콘텐츠 북마크 & 소비 추적)을 소개한다
- 새 도메인의 Value Object에 대해 SDD(Spec 정의)와 TDD(구현)를 함께 시연한다
...
- 콘텐츠 북마크 & 소비 추적 도메인으로 확정
- 디테일은 각 챕터 작성 시 구체화한다
