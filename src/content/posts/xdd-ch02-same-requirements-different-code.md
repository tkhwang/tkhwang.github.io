...
다음 글(Ch.3)에서는 DDD의 기본 개념을 먼저 정리합니다. 그리고 Ch.4에서 나머지 핵심 Building Block과 협력 구조를 이어서 정리한 뒤, Ch.5에서 요구사항을 PRD로 정리하고, Ch.6에서 DDD 관점으로 분해하고, Ch.7에서 Spec을 정의하고, Ch.8에서 테스트 케이스를 정리하고, Ch.9에서 TDD로 구현하는 흐름으로 들어갑니다.
