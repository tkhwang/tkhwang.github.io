...
이번 글에서는 누구나 익숙한 고객(Customer) 예제로 두 가지 사고방식을 비교합니다. 예시는 의도적으로 단순화했고, 핵심은 구현 디테일보다 모델링 관점의 차이에 있습니다. Part 2부터는 시리즈의 본 예제인 태스크/프로젝트 관리 도메인으로 들어갑니다.
...
class Customer {                         class Customer {
  ...
  points: number                           points: RewardPoints
}                                        }
```

* 왼쪽은 익숙합니다. `name`은 `string`, `email`은 `string`, `points`는 `number`.
* 오른쪽은 `Name`, `Email`, `RewardPoints`라는 별도의 객체를 사용합니다.
...
// UpdateCustomerNameDto — 같은 규칙을 다시 선언
...
`POST /customers`와 `PUT /customers/:id/name`에서 DTO 검증을 거치니까 문제될 상황이 없어 보입니다. **하지만 이 방어막만으로는 충분하지 않을 때가 있습니다.**
...
여기서 눈여겨볼 점이 있습니다. `new Name("홍길동")`이 아니라 `Name.create("홍길동")`으로 생성한다는 점입니다.
...
이 패턴은 이후 `RewardPoints`, `Email`에도 그대로 적용됩니다. **`create()`를 통과했다는 것 자체가 "이 값은 유효하다"는 보증**입니다.
...
잘못된 형식은 생성 자체가 불가능합니다. 공백과 대소문자도 생성 시점에 정규화됩니다.

여기서는 "이메일은 대소문자를 구분하지 않고 저장한다"는 제품 정책을 가정했습니다. 실제 시스템에서는 정규화 범위를 팀 정책으로 명확히 정하면 됩니다.

### 객체 위주 사고방식의 공통 원칙

세 경우 모두 같은 원칙을 따릅니다. **객체가 생성 시점에 규칙을 강제하고, 한 번 생성된 값은 그 규칙을 만족합니다.** 그래서 객체를 사용하는 쪽의 중복 검증을 크게 줄일 수 있습니다. DDD에서는 이처럼 도메인에서 의미 있는 값을 **Value Object**로 모델링합니다.
...
포인트를 더하는 로직이 Service에 있습니다. `earnedPoints`와 `bonusPoints`는 그냥 데이터이고, 어떻게 더해야 하는지는 외부가 알고 있습니다. 데이터는 수동적이고, 로직은 객체 밖에 있습니다.
...
이번 글에서 확인한 것은 **사고방식의 차이**입니다. 같은 요구사항이라도 데이터 그릇으로 다루느냐, 규칙과 행동을 품은 객체로 다루느냐에 따라 코드의 구조는 완전히 달라집니다.

다만 여기서 사용한 Customer 예제는 Value Object와 객체 위주 모델링의 차이를 빠르게 보여주기 위한 도입 예제입니다. SDD, TDD, DDD의 전체 흐름을 소개하는 단계에서는 이 정도 단순함이 오히려 도움이 됩니다.

이제 남은 질문은 이것입니다. 이 객체 위주 사고방식을 실제 설계와 구현으로 어떻게 이어 갈 것인가?

다음 글(Ch.3)에서는 DDD의 핵심 개념인 Value Object, Entity, Aggregate를 먼저 정리합니다. 그리고 그다음 글부터 요구사항에서 도메인 객체를 뽑아내고, Spec을 정의하고, TDD로 구현하는 흐름으로 들어갑니다.

Part 1에서는 "왜"와 "무엇이 다른가"를 확인했습니다. 이제 Part 2에서는 상태 전이, 일관성 경계, Aggregate 같은 개념이 더 분명하게 드러나는 복잡한 도메인으로 넘어가, "어떻게 만들 것인가"를 본격적으로 다룹니다.
